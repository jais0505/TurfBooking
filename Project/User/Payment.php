   ...
   <body>
      ...
      >
         <div class="cc__card">
			...
			$selbookamount="select * from tbl_booking where booking_id=".$_GET["bid"];
			$selresult=$Con->query($selbookamount);
			$seldata=$selresult->fetch_assoc();
			$bookingamount=$seldata["booking_amount"];
			?>
            <span style="color:white">₹ <?php echo $bookingamount ?></span>
		 ...
		 {
			$update="update tbl_booking set booking_status='1' where booking_id =".$_GET['bid'];
			if($Con->query($update))
			{
				?>
				<script>window.location = "Sucesser.php"</script>
				<?php
			}	 
		 }
		 ?>
